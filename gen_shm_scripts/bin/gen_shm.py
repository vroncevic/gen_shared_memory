...
class GenSHM(CfgBase):
    """
        Define class GenSHM with attribute(s) and method(s).
    ...
    def __init__(self, verbose=False):
        """
            ...
            :exceptions: None
        """
        verbose_message(GenSHM.VERBOSE, verbose, 'Initial configuration')
        current_dir = Path(__file__).resolve().parent
        base_config_file = "{0}{1}".format(current_dir, GenSHM.__CONFIG)
        CfgBase.__init__(self, base_config_file, verbose=verbose)
        if self.tool_status:
            self.add_new_option(
                GenSHM.__OPS[0], GenSHM.__OPS[1], dest="pro",
                help="generate SHM project skeleton"
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_status:
            ...
            if num_of_args == 1 and opts.pro and not project_exists:
                generator, gen_status = SHMSetup(verbose=verbose), False
                message = "{0} {1} [{2}]".format(
                    "[{0}]".format(self.name),
                    'Generating SHM project skeleton', opts.pro
                )
                print(message)
