...
from gen_shared_memory.infrastructure.cli.setup.validator import (
    CLIBundleValidator
)
...
from gen_shared_memory.infrastructure.command.icommand_executor import (
    ICommandExecutor
)
...
class CLI:
    ...
    _executors: Mapping[
        str, ICommandExecutor[ICommandDefinition, object, object, object]
    ]
    ...
    def __init__(self, bundle: CLIBundle) -> None:
        ...
        self._parser.register_commands(
            [pair.definition for pair in bundle.commands]
        )
    ...
    def run(self) -> Mapping[str, object]:
        ...
        try:
            ...
            executor: (
                ICommandExecutor[ICommandDefinition, object, object, object]
                | None
            ) = self._executors.get(command_name)

            return (
                executor.execute(params=params, service=self._service)
                if executor
                else {
                    'returncode': 1,
                    'stdout': '',
            ...
            return {
                'returncode': 1,
                'stdout': '',
                'stderr': f'cli::run - command execution failed {exc}'
            }
    ...
    def is_initialized(self) -> bool:
        ...
        '''
        return (
            self._service.is_initialized()
            and self._parser.is_initialized()
            and all(bool(executor) for executor in self._executors.values())
        )
