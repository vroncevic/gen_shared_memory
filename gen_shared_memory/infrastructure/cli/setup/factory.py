...
from gen_shared_memory.infrastructure.cli.setup.options import CLIBundleOptions
from gen_shared_memory.infrastructure.cli.setup.opt_validator import CLIBundleOptionsValidator
from gen_shared_memory.infrastructure.cli.setup.bundle import CLIBundle
from gen_shared_memory.infrastructure.cli.setup.keys import CLIBundleKeys
...
from gen_shared_memory.infrastructure.command.icommand_definition import ICommandDefinition
from gen_shared_memory.infrastructure.command.icommand_executor import ICommandExecutor
from gen_shared_memory.infrastructure.command.gen_shared_memory_command_definition import (
    GenSharedMemoryCommandDefinition
)
from gen_shared_memory.infrastructure.command.gen_shared_memory_command_executor import (
    GenSharedMemoryCommandExecutor
)
...
class CLIBundleFactory:
    ...
    def create_bundle(cls, options: CLIBundleOptions) -> CLIBundle:
        ...
        service: IService | None = (
            options.get(CLIBundleKeys.OPTION_SERVICE) if options else None
        )
        parser: IOptionManager | None = (
            options.get(CLIBundleKeys.OPTION_PARSER) if options else None
        )

        service_definition: ICommandDefinition = GenSharedMemoryCommandDefinition()
        service_executor: ICommandExecutor = GenSharedMemoryCommandExecutor(
            definition=service_definition
        )
        service_cmd: CommandBundle = CommandBundle(
            definition=service_definition, executor=service_executor
        )
