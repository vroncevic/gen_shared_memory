...
class SubProcessor:
    ...
    def __init__(self, generator: IGeneratorManager) -> None:
        ...
        msg_generator_istype: str = (
            f'the generator must be an instance of {IGeneratorManager.__name__}'
        )
    ...
    def run(self, *, params: Mapping[str, object]) -> Mapping[str, object]:
        '''
            ...
            :exceptions: None.
        '''
        project_name: str = 'myshm'
        try:
            current_dir: str = dirname(realpath(__file__))
            output_dir: str = str(params.get('output') or './')
            project_name = str(params.get('name') or 'myshm')
            ...
            return {
                'returncode': 1,
                'stdout': '',
                'stderr': f'failed to generate {project_name} project {exc}'
            }
