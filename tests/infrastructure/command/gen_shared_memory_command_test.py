...
'''
Module
    gen_shared_memory_command_test.py
Info
    Unit tests for GenSharedMemoryCommandDefinition and GenSharedMemoryCommandExecutor.
'''
...
class TestGenSharedMemoryCommand(TestCase):

    def test_definition(self) -> None:
        definition = GenSharedMemoryCommandDefinition()
        self.assertEqual(definition.name, 'create')
        self.assertEqual(
            definition.help_text, 'Generate shared memory project files'
        )
    ...
    def test_executor_execute_success(self) -> None:
        definition = GenSharedMemoryCommandDefinition()
        executor = GenSharedMemoryCommandExecutor(definition)
